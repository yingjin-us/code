...
CREATE         procedure [dbo].[usp_Paid_leave_benefit_eligibility_check_2]
			...
			BEGIN
				Insert into crs_tbl_Paid_leave_benefit_py_checks(Empl_ID, fy_cd, hours, IsEligible )
					select @EmplID , service_year, sum(hours),
						case	when @fycd = service_year and sum(hours)>=@MinQaulifyingHours then 1 
								when @fycd <> service_year and sum(hours)>=@MinQfyPrevHrs then 1 
								else 0 end iselig 
