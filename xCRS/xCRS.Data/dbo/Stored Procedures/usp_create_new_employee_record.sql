...
CREATE          procedure [dbo].[usp_create_new_employee_record]
...
--then just update the employee active =1 and archived=0
	...
	--print 'GRADE' + @grade
	set @mcount=(select count(*) from crs_tblSal_Info where empl_id=@empl_id)	
	if @grade<='15' and @mcount = 0
			...
			and e.empl_id = @empl_id  order by e.empl_id, effect_dt desc 
	else if @grade is NULL and @mcount = 0
			--print 'ok'
			insert into crs_tblSal_Info (
				empl_id, effect_dt, hur_rate, sal, sal_inc,per_inc, 
				grade, grade_step, change_code, updateid, updatedate, ReviewYear) 
...
/*		
		update CRS_tmpReport_Downloaded
			...
			case 	
				...
				when cur_grade=16 then 
					case 	when cur_sal < 110.0 then '16L' 
						else '16U'
					END 
		...
		where 
			...
			and org like '%.1'
			and CRS_tmpReport_Downloaded.empl_id = @Empl_ID
