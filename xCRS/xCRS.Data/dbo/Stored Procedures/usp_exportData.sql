CREATE                              procedure [dbo].[usp_exportData]
...
*/
select d.EmployeeID as "Westat ID", v.Org_ID, v.Last_Name as "Last Name"
, v.First_Name as "First Name", d.DegreeID as Degree, d.DegreeType as Description
, Major, Minor, School as Instutution
, Concentration, DegreeDate AS [COMPLETED DATE] 
from issql1.wisdom.dbo.EmployeeDegree d, vw_Empl_Info v
where d.EmployeeID = v.empl_id
...
select v.empl_id as "Westat ID", v.org_id, v.Last_Name, v.First_Name
, w.OrganizationName as "PRIOR EMPLOYER"
, w.Title as "PRIOR POSITION"
, w.StartDate as [START DATE OF EMPLOYMENT]
, w.EndDate as [END DATE OF EMPLOYMENT]
, DateDiff(Year,StartDate,EndDate) as YEARS
from issql1.wisdom.dbo.EmployeeWorkExperience w, vw_Empl_Info v
where w.EmployeeID = v.empl_id
...
FROM         dbo.CRS_tblSal_Info INNER JOIN
	...
	select v.EMPL_ID,LAST_NAME,FIRST_NAME,TERM_DT,
		...
		FullTime_Percent,
		DOSE,DOR, 
		v.title_code, Grade_Category, flsa_cd FROM 
	        dbo.vw_AllStaffSalary_Info v, dbo.DELTEK_EMPL_LAB_INFO l, idmaster i, (select empl_id, flsa_cd from dbo.vw_Empl_Info_BrowseOnly) b
		where l.empl_id = v.empl_id and l.end_dt = '31-Dec-2078'
			and i.idmaster_westatid = v.empl_id
			and i.idmaster_westatid = b.empl_id
	...
	select [User_ID]      ,[Empl_ID]      ,[LName]      ,[FName]      ,[MIName]      ,[DOSE]      ,[DOPE]
      ,[DOT]      ,[Active]      ,[ORG]      ,[Rev_Code]      ,[Grade_Category]      ,[SU1]      ,[SU1_Name]
      ,[SU2]      ,[SU2_Name]      ,[SU3]      ,[SU3_Name]      ,[SU4]      ,[SU4_Name]      ,[SI1]      ,[PI1]
      ,[GR1]      ,[RCol1]      ,[Row1]      ,[Bon1]      ,[BonType1]      ,[SI2]      ,[PI2]      ,[GR2]      ,[RCol2]
      ,[Row2]      ,[Bon2]      ,[BonType2]      ,[SI3]      ,[PI3]      ,[GR3]      ,[RCol3]      ,[Row3]      ,[Bon3]
      ,[BonType3]      ,[SI4]      ,[PI4]      ,[RCol4]      ,[Row4]      ,[Bon4]      ,[BonType4]      ,[Cur_Sal]
      ,[New_Sal]      ,[Cur_Grade]      ,[Cur_GR_Step]      ,[New_Grade]      ,[New_GR_Step]      ,[Cur_Bon]
      ,[Cur_Rate]      ,[TimeStamp]      ,[YearVal]      ,[Util_Percent]      ,[FullTime_Percent]      ,[Sal2]
      ,[Sal3]      ,[Sal_Inc_Tot]      ,[GR_Step1]      ,[GR_Step2]      ,[Grade_Step_Min3]      ,Replace([Notes],char(13) + char(10),'') Notes
      ,[Perf_Rating]      ,[Growth_Rating]      ,[Directory_Title]      ,[Sub_Unit]      ,[Family_code]
      ,[Hiring_commitement]      ,[PTS]      ,[OrgName]      ,[Total_BonusForPreviousYears]      ,[BudgetGrade]
      ,[BudgetSalary]      ,[BudgetBonus]      ,[CompReviewStatus]      ,[InitialCompReviewStatus]      ,[AllStaffBon]
      ,[Years_Of_Experience] from crs_tmpreport_downloaded
...
else if @tbl = 'employeesinfowithnotes'
