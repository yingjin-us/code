...
create    VIEW  vw_R_Comp_Report_With_Bonus_backup
AS
SELECT     CRS_tmpReport.User_ID, CRS_tmpReport.Empl_ID,  CRS_tmpReport.LName,  CRS_tmpReport.FName,  CRS_tmpReport.MIName, 
	              ...
	              CRS_tmpReport.Cur_Bon,  CRS_tmpReport.Cur_Rate,  CRS_tmpReport.YearVal,  
			--CRS_tmpBonType.All_Bon_Type, 
                      --CRS_tmpBonType.Sum_Bon_Amt,   
			vw_R_Bon_Type_BSA.Bon_Type AS Bon_Type_BSA, ISNULL( vw_R_Bon_Type_BSA.Bon_Amt, 0) 
...
FROM          	      
		CRS_tmpReport_downloaded CRS_tmpReport 
			...
			LEFT OUTER JOIN  vw_R_Bon_Type_BSA ON  CRS_tmpReport.Empl_ID =  vw_R_Bon_Type_BSA.Empl_ID AND 
                      						CRS_tmpReport.YearVal =  vw_R_Bon_Type_BSA.Effect_Year 
			LEFT OUTER JOIN vw_R_Bon_Type_BNH ON  CRS_tmpReport.Empl_ID =  vw_R_Bon_Type_BNH.Empl_ID AND 
                      						CRS_tmpReport.YearVal =  vw_R_Bon_Type_BNH.Effect_Year 
			LEFT OUTER JOIN vw_R_Bon_Type_BST ON  CRS_tmpReport.Empl_ID =  vw_R_Bon_Type_BST.Empl_ID AND 
                      						CRS_tmpReport.YearVal =  vw_R_Bon_Type_BST.Effect_Year 
			
			--LEFT OUTER JOIN CRS_tmpBonType ON  CRS_tmpReport.Empl_ID =  CRS_tmpBonType.Empl_ID
			--where SI4 is not null
			--where CRS_tmpReport.LName like 'ROSEN%'
